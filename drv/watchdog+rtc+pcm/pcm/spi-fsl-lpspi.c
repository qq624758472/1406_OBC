...
{
	//printk(KERN_EMERG "fsl_lpspi_write_tx_fifo is going\n");
	u8 txfifo_cnt;
	u32 temp;
	...
	//	printk(KERN_EMERG "fsl_lpspi_intctrl(fsl_lpspi, IER_FCIE)\n");
	//	printk(KERN_EMERG "SPI EMERGENCY DEBUG: SR=0x%08x, TCR=0x%08x, IER=0x%08x\n",
   // readl(fsl_lpspi->base + IMX7ULP_SR),
    //readl(fsl_lpspi->base + IMX7ULP_TCR),
   // readl(fsl_lpspi->base + IMX7ULP_IER));
	} else{
		fsl_lpspi_intctrl(fsl_lpspi, IER_TDIE);
          int ret;
       // ret = fsl_lpspi_wait_for_completion(controller);
	//	printk(KERN_EMERG "fsl_lpspi_intctrl(fsl_lpspi, IER_TDIE)\n");
	//	    printk(KERN_EMERG "SPI EMERGENCY DEBUG: SR=0x%08x, TCR=0x%08x, IER=0x%08x\n",
    //readl(fsl_lpspi->base + IMX7ULP_SR),
    //readl(fsl_lpspi->base + IMX7ULP_TCR),
   // readl(fsl_lpspi->base + IMX7ULP_IER));
...
static void fsl_lpspi_set_cmd(struct fsl_lpspi_data *fsl_lpspi)
{
    u32 temp = 0;
    ////////////////////////////////////////
    //fsl_lpspi->usedma = 0;  // 全局禁用 DMA
    bool usedma = fsl_lpspi->usedma;  // 临时保存，便于打印

   // printk(KERN_INFO "SPI: fsl_lpspi_set_cmd called\n");
   // printk(KERN_INFO "SPI: bpw=%d, mode=0x%x, chip_select=%d, prescale=%d\n",
   //        fsl_lpspi->config.bpw,
   //        fsl_lpspi->config.mode,
   //        fsl_lpspi->config.chip_select,
   //        fsl_lpspi->config.prescale);
   // printk(KERN_INFO "SPI: is_slave=%d, usedma=%d, is_first_byte=%d\n",
   //        fsl_lpspi->is_slave, usedma, fsl_lpspi->is_first_byte);
/////////////////////////////////////////////
...
////////////////////////////////////////////////////////////////////////
    // 添加最终 temp 值的打印（用 printk 确保可见）
   // printk(KERN_INFO "SPI: Final TCR register value = 0x%08x\n", temp);
}
...
static int fsl_lpspi_wait_for_completion(struct spi_controller *controller)
{
	...
	//printk(KERN_EMERG "fsl_lpspi_wait_for_completion is going\n");
   // printk(KERN_EMERG "SPI EMERGENCY DEBUG: SR=0x%08x, TCR=0x%08x, IER=0x%08x\n",
   // readl(fsl_lpspi->base + IMX7ULP_SR),
   // readl(fsl_lpspi->base + IMX7ULP_TCR),
   // readl(fsl_lpspi->base + IMX7ULP_IER));
	if (fsl_lpspi->is_slave) {
		...
		//if (!wait_for_completion_timeout(&fsl_lpspi->xfer_done, HZ)) {
			...
			//printk(KERN_EMERG "wait for completion timeout\n");
  //  printk(KERN_EMERG "SPI EMERGENCY DEBUG: SR=0x%08x, TCR=0x%08x, IER=0x%08x\n",
   // readl(fsl_lpspi->base + IMX7ULP_SR),
   // readl(fsl_lpspi->base + IMX7ULP_TCR),
   // readl(fsl_lpspi->base + IMX7ULP_IER));
	...
	temp_SR = readl(fsl_lpspi->base + IMX7ULP_SR);
     while ((temp_SR&(SR_FCF))==0) {
 	temp_SR = readl(fsl_lpspi->base + IMX7ULP_SR);
 	cnt ++;
 	if(cnt > 100000)
 	{
 		//printk(KERN_EMERG "time out.\n");
 		return -ETIMEDOUT;
 	}
     }
     		//	printk(KERN_EMERG "2wait for completion timeout2\n");
    //printk(KERN_EMERG "SPI EMERGENCY DEBUG: SR=0x%08x, TCR=0x%08x, IER=0x%08x\n",
   // readl(fsl_lpspi->base + IMX7ULP_SR),
    //readl(fsl_lpspi->base + IMX7ULP_TCR),
    //readl(fsl_lpspi->base + IMX7ULP_IER));
    		...
    		fsl_lpspi_read_rx_fifo(fsl_lpspi);
              // printk(KERN_EMERG "read out. cnt=%d!\n", cnt);
               //if(temp_SR&SR_TDF){
              // printk(KERN_EMERG "SR_TDF!\n");
              // 	fsl_lpspi_write_tx_fifo(fsl_lpspi);
              //	return 0;
            //   }

	       if(temp_SR&SR_FCF){
              // printk(KERN_EMERG "SR_FCF!\n");
...
static int fsl_lpspi_pio_transfer(struct spi_controller *controller,
				  struct spi_transfer *t)
{
	//printk(KERN_EMERG "fsl_lpspi_pio_transfer is going\n");
	struct fsl_lpspi_data *fsl_lpspi =
				spi_controller_get_devdata(controller);
	int ret;
...
static int fsl_lpspi_transfer_one(struct spi_controller *controller,
				  ...
				  struct spi_transfer *t)
{
 	//printk(KERN_EMERG "fsl_lpspi_transfer_one is going\n");
	struct fsl_lpspi_data *fsl_lpspi =
					spi_controller_get_devdata(controller);
	int ret;
	...
	//fsl_lpspi_read_rx_fifo(fsl_lpspi);
	while (!(readl(fsl_lpspi->base + IMX7ULP_RSR) & RSR_RXEMPTY))
		readl(fsl_lpspi->base + IMX7ULP_RDR);
	//printk(KERN_EMERG"clear rx fifo: RSR=0x%08x\n", readl(fsl_lpspi->base + IMX7ULP_RSR));
	if (fsl_lpspi->usedma)
...
static irqreturn_t fsl_lpspi_isr(int irq, void *dev_id)
{
    ...
    // 👉 加调试：打印进入中断
    printk(KERN_EMERG"fsl_lpspi_isr: enter\n");
    ...
    // 👉 加调试：打印关键寄存器
    printk(KERN_EMERG"ISR: IER=0x%x, SR=0x%x\n", temp_IER, temp_SR);

    // ❌ 不要立即关中断！改为先处理
    // fsl_lpspi_intctrl(fsl_lpspi, 0);  // 移到后面或去掉

    fsl_lpspi_read_rx_fifo(fsl_lpspi);

    if ((temp_SR & SR_TDF) && (temp_IER & IER_TDIE)) {
       printk(KERN_EMERG"ISR: TDF interrupt, TXCOUNT=%d\n", 
                (readl(fsl_lpspi->base + IMX7ULP_FSR) & FSR_TXCOUNT));
        fsl_lpspi_write_tx_fifo(fsl_lpspi);
        goto out;  // 处理完再统一关中断
    }
    ...
    if (temp_SR & SR_MBF ||
        readl(fsl_lpspi->base + IMX7ULP_FSR) & FSR_TXCOUNT) {
        printk(KERN_EMERG"ISR: MBF or TXCOUNT, enable FCF int\n");
        writel(SR_FCF, fsl_lpspi->base + IMX7ULP_SR);
        fsl_lpspi_intctrl(fsl_lpspi, IER_FCIE);
        goto out;
    }
    ...
    if (temp_SR & SR_FCF && (temp_IER & IER_FCIE)) {
        printk(KERN_EMERG"ISR: FCF interrupt! Transfer complete!\n");
        writel(SR_FCF, fsl_lpspi->base + IMX7ULP_SR);
        complete(&fsl_lpspi->xfer_done);
        goto out;
    }

    printk(KERN_EMERG"ISR: unknown interrupt! IER=0x%x, SR=0x%x\n", temp_IER, temp_SR);
    return IRQ_NONE;
...
static int fsl_lpspi_probe(struct platform_device *pdev)
{
	...
	irq = platform_get_irq(pdev, 0);
	 printk(KERN_INFO"fsl_lpspi: platform_get_irq returned %d\n", irq);
	if (irq < 0) {
		ret = irq;
		goto out_controller_put;
	}
	printk(KERN_INFO"fsl_lpspi: got irq = %d\n", irq);
	printk(KERN_INFO"fsl_lpspi: requesting irq...\n");
	ret = devm_request_irq(&pdev->dev, irq, fsl_lpspi_isr, 0,
			       dev_name(&pdev->dev), fsl_lpspi);
	if (ret) {
	   printk(KERN_INFO"fsl_lpspi: request_irq failed with %d\n", ret);
	} else {
	   printk(KERN_INFO"fsl_lpspi: request_irq success!\n");
	}
	if (ret) {
