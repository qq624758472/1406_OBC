...
//  使用指令用法：
//  ./a.out -c <PWM通道编号: 1~6> -t <pwm[REG_PRESCALE]>  -f <pwm[REG_PERIOD]> -p <pwm[REG_PWM1_POSEDGE]> -n <pwm[REG_PWM1_NEGEDGE]>  
//  使用示例： ./a.out -c  1 -t 0x13 -f 0x63 -p 0x01 -n 0x01 , 
//  除了第一个参数，其余参数使用16进制格式，如果未指定都是代码中默认的。
